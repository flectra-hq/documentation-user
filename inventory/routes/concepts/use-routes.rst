...
following the instructions given by the rules. Yes, Flectra simplifies your
life.
...
order, a sale order. So, Flectra generates a need at the *Customer
Location* for each product in the order. Because *Pull Rules*
are triggered by a need, Flectra looks for a *Pull Rule* defined on the
...
Then, Flectra finds another pull rule that tries to fulfill the need for
...
.. note::
   All theses transfers are pre-generated by Flectra, starting from the end and going backwards.
...
Since *Routes* are a collection of *Push and Pull Rules*, Flectra
...
.. image:: media/multi-steps-routes-feature.png
   :align: center
   :alt: activation of the multi-steps feature in Flectra Inventory
...
Flectra’s pre-configured routes are available in the warehouses’ menu. To
...
.. image:: media/example-preconfigured-warehouse.png
   :align: center
   :alt: preconfigured warehouse in Flectra Inventory
...
Configuration --> Routes` to see the routes Flectra generated for you.
...
.. image:: media/rules-example.png
   :align: center
   :alt: rules example with push & pull actions in Flectra Inventory
...
The available rules trigger various actions. If Flectra offers *Push* and
...
-  **Pull From**: this rule is triggered by a need for the product
   ...
   source location, Flectra generates a picking to fulfill this need.
-  **Push To**: this rule is triggered by the arrival of some
   products in the defined source location. In case you move
   products to the source location, Flectra generates a picking to move
...
In the *Scheduling* section, you can determine how Flectra behaves when
