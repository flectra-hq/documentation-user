...
source_read_replace_vals = {
    ...
    'GITHUB_PATH': f'https://github.com/flectra/flectra/blob/{version}',
    'GITHUB_ENT_PATH': f'https://github.com/flectra/enterprise/blob/{version}',
    'OWL_PATH': f'https://github.com/flectra/owl/blob/master',
}
...
odoo_sources_candidate_dirs = (Path('flectra'), Path('../flectra'))
odoo_sources_dirs = [
    d for d in odoo_sources_candidate_dirs if d.is_dir() and (d / 'flectra-bin').exists()
]
...
if not odoo_sources_dirs:
    _logger.warning(
        ...
        "`git clone https://github.com/flectra/flectra` or create a symbolic link.",
        {'dir_list': '\n'.join([f'\t- {d.resolve()}' for d in odoo_sources_candidate_dirs])},
    )
else:
    if (3, 6) < sys.version_info < (3, 7):
        # Running flectra needs python 3.7 min but monkey patch version_info to be compatible with 3.6.
        sys.version_info = (3, 7, 0)
    odoo_dir = odoo_sources_dirs[0].resolve()
    source_read_replace_vals['ODOO_RELPATH'] = '/../' + str(odoo_sources_dirs[0])
    sys.path.insert(0, str(odoo_dir))
    import flectra.addons
    flectra.addons.__path__.append(str(odoo_dir) + '/addons')
    ...
    if release != odoo_version:
        _logger.warning(
            ...
            " with `cd flectra && git checkout %(doc_version)s`.",
...
# Mapping between flectra models related to master data and the declaration of the
...
model_references = {
    ...
    'res.country': 'flectra/addons/base/data/res_country_data.xml',
    'res.currency': 'flectra/addons/base/data/res_currency_data.xml',
}
...
latex_additional_files = ['static/latex/flectra.sty']
