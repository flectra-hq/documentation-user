...
This chapter helps you to create your first Flectra module and deploy it in your Flectra.sh project.

This tutorial requires :ref:`you created a project on Flectra.sh <odoosh-gettingstarted-create>`, and
...
* *~/src* is the directory where are located the Git repositories related to your Flectra projects,
* *flectra* is the Github user,
* *flectra-addons* is the Github repository,
...
.. code-block:: bash
  ...
  $ mkdir ~/src
  $ cd ~/src
  $ git clone https://github.com/flectra/flectra-addons.git
  $ cd ~/src/flectra-addons
...
While not necessary, scaffolding avoids the tedium of setting the basic Flectra module structure.
You can scaffold a new module using the executable *flectra-bin*.

From the Flectra.sh editor, in a terminal:

.. code-block:: bash

  $ flectra-bin scaffold my_module ~/src/user/

Or, from your computer, if you have an :ref:`installation of Flectra <setup/install/source>`:

.. code-block:: bash

  $ ./flectra-bin scaffold my_module ~/src/flectra-addons/

If you do not want to bother installing Flectra on your computer,
...
From an Flectra.sh editor terminal:
...
If your Flectra.sh project is configured to install your module automatically,
...
If you would like the update to be performed automatically by the Flectra.sh platform when you push
...
Then, from an Flectra.sh terminal:
...
drag-and-dropping the branch on the production branch in the Flectra.sh interface. As you increased the
...
.. note::
   - It is not possible to install or upgrade system packages on an Flectra.sh database (e.g., apt
     ...
     **third-party Flectra modules**.
   - **PostgreSQL extensions** are not supported on Flectra.sh.
   - For more information, consult our `FAQ <https://www.flectra.sh/faq#install_dependencies>`_.
...
From the Flectra.sh editor, create and open the file ~/src/user/requirements.txt.

Or, from your computer, create and open the file ~/src/flectra-addons/requirements.txt.
