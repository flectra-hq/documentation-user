...
Flectra.sh offers three different stages for your branches: production, staging and development.
...
The unit tests are not performed as, in Flectra, they currently rely on the demo data, which is not loaded in the
production database. In the future, if Flectra supports to run the unit tests without the demo data,
Flectra.sh will then consider running the tests on staging databases.
...
Flectra.sh will be notified when new revisions have been pushed in your branches.
...
You can also open terminals, Python consoles and even Flectra Shell consoles.
...
* flectra.log: The logs of the running server.
...
Flectra.sh makes daily backups of the production database. It keeps 7 daily, 4 weekly and 3 monthly backups.
...
To restore or download one of these monthly backups, please `contact us <https://www.flectra.com/help>`_.

If you merge a commit updating the version of one or several modules (in :file:`__manifest__.py`), or their linked python
dependencies (in :file:`requirements.txt`), then Flectra.sh performs a backup automatically (flagged with type Update in the list),
...
by Flectra.sh, as neither the container nor the database is modified so the platform considers this safe enough. Of course,
...
* the standard Flectra databases manager,
  (available for on-premise Flectra servers under :code:`/web/database/manager`)
* the Flectra online databases manager,
* the Flectra.sh backup download button of this *Backups* tab,
* the Flectra.sh dump download button in the :ref:`Builds view <odoosh-gettingstarted-builds>`.
...
.. seealso::
    :doc:`Upgrade - Flectra.sh <../../upgrade/odoo_sh>`
...
* *Full installation (all modules)* will install the modules of the branch, the modules included in the submodules
  and all standard modules of Flectra. When running the full installation, the test suite is disabled.
...
If the tests are enabled, the standard Flectra modules suite can take up to 1 hour.
...
For development branches only, you can change the version of Flectra, should you want to test upgraded code or develop
...
* You can choose to benefit from the latest bug, security and performance fixes automatically. The
  sources of your Flectra server will be updated weekly. This is the 'Latest' option.
* You can choose to pin the Flectra sources to a specific revision by selecting them from a list of
...
*<name>.flectra.com* domains or your own custom domains. For the latter you have to:
...
For instance, to associate *www.mycompany.com* to your database *mycompany.flectra.com*:

* in Flectra.sh, add *www.mycompany.com* in the custom domains of your project settings,
* in your domain name manager (e.g. *godaddy.com*, *gandi.net*, *ovh.com*),
  configure *www.mycompany.com* with a ``CNAME`` record with as value *mycompany.flectra.com*.
...
While it is currently not possible to configure your own SSL certificates on the Flectra.sh platform
...
(DomainKeys Identified Mail), don't forget to authorize Flectra as a sending host in your domain name
...
.. Warning::
  Forgetting to configure your SPF or DKIM to authorize Flectra as a sending host can lead to the
...
and some of them can be used directly from Flectra.sh by clicking the *run* button
...
.. code-block:: bash
  ...
  $ git clone --recurse-submodules --branch master git@github.com:flectra/flectra.git

Clones the repository *flectra/flectra*.
