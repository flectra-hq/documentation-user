...
Go to `Flectra.sh <https://www.flectra.sh/>`_ and hit the *Deploy your platform* button.
...
Authorize Flectra.sh
====================

Grant Flectra.sh the required accesses to your account by clicking the *Authorize* button.

.. image:: create/github-authorize.png
   :align: center

Flectra.sh basically needs:
...
Choose the Flectra version you want to use. If you plan to import an existing database or an existing
...
It should be the code of your Enterprise subscription that includes Flectra.sh.

Partners can use their partnership codes to start a trial. Should their clients start a project,
they ought to get an Enterprise subscription including Flectra.sh and use its subscription code. The
...
* it is an enterprise subscription, but which does not include Flectra.sh,
* it is neither a partnership subscription or an enterprise subscription (e.g. an online
  subscription).

In case of doubt with your subscription, please contact the `Flectra support
...
You can start using Flectra.sh. Your first build is about to be created. You will soon be able to
...
You can import your database in your Flectra.sh project as long as it is in a :doc:`supported version
</administration/maintain/supported_versions>` of Flectra.

Push your modules in production
-------------------------------

If you use community or custom modules, add them in a branch in your Github repository.
Databases hosted on the Flectra.com online platform do not have any custom modules.
Users of these databases can therefore skip this step.

You can structure your modules as you wish, Flectra.sh will automatically detect the folders containing
Flectra addons. For instance, you can put all your modules folder in the root directory of your
...
Flectra Online databases
~~~~~~~~~~~~~~~~~~~~~~~~

`Access your databases manager <https://accounts.flectra.com/my/databases/manage>`_ and download a
...
.. Warning::
  ...
  Online versions (e.g. *saas-**) are not supported on Flectra.sh.

Upload the backup
-----------------

Then, in your Flectra.sh project, in the backups tab of your production branch, import the backup you
just downloaded.
...
There is a default mail server provided with Flectra.sh.
...
After the import of your database, all outgoing email servers are disabled so you use the Flectra.sh
