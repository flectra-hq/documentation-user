...
listening on :option:`the gevent port <flectra-bin --gevent-port>` but
...
* Deploy a threaded version of Flectra (instead of a process-based preforking
  one) and redirect only requests to URLs starting with ``/websocket/`` to
  that Flectra, this is the simplest and the websocket URL can double up as the cron
  instance.
* Deploy an evented Flectra via ``flectra-gevent`` and proxy requests starting
  with ``/websocket/`` to
  :option:`the gevent port <flectra-bin --gevent-port>`.
...
For development convenience, Flectra directly serves all static files and attachments in its modules.
...
Flectra static files are located in each module's :file:`static/` folder, so static files can be served
...
.. example::
   ...
   .. code-block:: nginx
       ...
       location ~ ^/[^/]+/static/.+$ {
           root /usr/lib/python3/dist-packages/flectra/addons;
           try_files $uri @flectra;
           expires 24h;
       }
...
Attachments are files stored in the filestore which access is regulated by Flectra. They cannot be
...
Nevertheless, once the file has been located and the access rights verified by Flectra, it is a good
idea to serve the file using the static web server instead of Flectra. For Flectra to delegate serving
...
must be enabled and configured on the static web server. Once it is set up, start Flectra with the
:option:`--x-sendfile <flectra-bin --x-sendfile>` CLI flag (this unique flag is used for both
   ...
   - The X-Accel extension for NGINX **does** require the following additionnal configuration:
     ...
     .. code-block:: nginx
         ...
         location /web/filestore {
             internal;
             alias /path/to/flectra/data-dir/filestore;
         }

     In case you don't know what is the path to your filestore, start Flectra with the
     :option:`--x-sendfile <flectra-bin --x-sendfile>` option and navigate to the ``/web/filestore`` URL
     directly via Flectra (don't navigate to the URL via NGINX). This logs a warnings, the message
