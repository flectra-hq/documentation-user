...
as it allows users to type a meaningful web address, such as ``www.flectra.com``, rather than its
server's IP address with a series of numbers.

You can use a custom domain name to access your Flectra database and websites:

- By :ref:`registering a free domain name with Flectra <domain-name/flectra-register>` (for Flectra Online
  databases)
- By :ref:`configuring a custom domain that you already own <domain-name/existing>`.

.. note::
   Flectra Online and Flectra.sh databases, including their websites, use by default a subdomain of
   ``flectra.com`` for both the URL and the emails (e.g., ``https://example.flectra.com``).

.. important::
   Flectra offers a :ref:`free custom domain name <domain-name/flectra-register>` to all Flectra Online
   databases for one year. Visitors can then access your website with an address such as
   ``www.example.com`` rather than the default ``example.flectra.com``.

.. seealso::
   - `Flectra Tutorials: Register a Free Domain Name
     <https://www.flectra.com/slides/slide/register-a-free-domain-name-1663>`_
   - `Flectra Quick Tips: Get a free domain name! <https://www.youtube.com/watch?v=eAfgeNOHLP4>`_
...
other websites than your main website or to specific pages (e.g., ``experience.flectra.com`` points to
a specific page.)
...
:ref:`domain names registered with Flectra <domain-name/flectra-manage>` and for domain names managed by
other providers.
...
.. _domain-name/flectra-register:

Register a free domain name with Flectra
========================================

You can register a domain name for your Flectra Online database directly from Flectra Website or your
database manager.

.. important::
   - Your domain name is **free for one year** if you register it with Flectra!
   ...
   - Flectra manages payment and technical support for you.
   - This offer doesn't include any mailbox. However, you can :ref:`configure your MX records
     <domain-name/flectra-manage>` to use your own email server or solution such as Google Workspace.

To do so, go to :menuselection:`Website --> Domain Name`. Alternatively, open your `database manager
<https://www.flectra.com/my/databases>`_, click on the :guilabel:`settings` button next to your
database, then on :guilabel:`Domain names`.

.. image:: domain_names/register-menu.png
   :align: center
   :alt: Clicking on Domain Names from an Flectra website
...
.. note::
   - Free domain names are also available for free Flectra Online databases (if you installed one app
     only, for example). In this case, Flectra reviews your request and your website to avoid abuse.
     This process may take up to three days.
   - This is not available for Flectra.sh databases yet.

.. _domain-name/flectra-manage:

Manage your domain name registered with Flectra
-----------------------------------------------

To manage the DNS records of your domain name registered with Flectra or to visualize the contacts
associated with it, open your `database manager <https://www.flectra.com/my/databases>`_, click on the
...
.. image:: domain_names/manage.png
   :align: center
   :alt: Management of the domain names linked to an Flectra database

.. note::
   Please `submit a support ticket <https://www.flectra.com/help>`_ if you need further assistance to
...
#. :ref:`Map your domain name with your Flectra database <domain-name/db-map>`.
#. :ref:`Map your domain name with your Flectra website <domain-name/website-map>`.
...
You need a CNAME record that points to your Flectra database. The requirements are detailed in your
database manager.
...
While Flectra suggests creating a CNAME record for your ``www.`` subdomain (``www.example.com``), you
can of course use any domain name of your choice, with any subdomain (e.g.,
``anything.example.com``).

.. example::
   You own the domain name ``example.com``, and you have an Flectra Online database at the address
   ``example.flectra.com``. You want to access your Flectra database primarily with the domain
   ``www.example.com`` but also with the :ref:`naked domain <domain-name/naked-domain>`
   ``example.com``.

   To do so, you create a CNAME record for the ``www`` subdomain, with ``example.flectra.com`` as the
   target. The DNS zone manager generates the following rule and adds it to your DNS zone: ``www IN
   CNAME example.flectra.com.``
...
(e.g., ``flectra.com`` instead of ``www.flectra.com``).
...
Map your domain name with your Flectra database
-----------------------------------------------
...
.. tabs::
   ...
   .. group-tab:: Flectra Online
      ...
      Open your `database manager <https://www.flectra.com/my/databases>`_, click on the
      :guilabel:`settings` button next to your database, on :guilabel:`Domain names`, and then on
   ...
   .. group-tab:: Flectra.sh
      ...
      .. image:: domain_names/odoo-sh-map.png
         :align: center
         :alt: Mapping a domain name with an Flectra.sh branch

      .. seealso::
         - :ref:`Flectra.sh branches: settings tab <odoosh-gettingstarted-branches-tabs-settings>`

.. warning::
   Make sure to :ref:`add a CNAME record <domain-name/cname>` to your domain name's DNS **before**
   mapping your domain name with your Flectra database.
   ...
   ticket <https://www.flectra.com/help>`_ including screenshots of your CNAME records.
...
Flectra generates a separate SSL certificate for each domain :ref:`mapped in the database manager
...
.. note::
   ...
   - If you already use another service, you can keep using it or simply change for Flectra.
...
.. note::
   If you have Flectra Website, you can disregard this part and directly :ref:`map your domain name
...
To configure it, access your Flectra database with your custom address, then log in as an administrator
of your database (any user in the *Settings* group) from the login screen.

.. warning::
   Connecting to your database with the original Flectra subdomain address (e.g., ``example.flectra.com``
...
.. warning::
   Mapping your domain name with your Flectra website prevents Google from indexing both your custom
   domain name ``www.example.com`` and your original flectra database address ``example.flectra.com``.
...
.. note::
   ...
   so indicates Flectra which URL to use as the :ref:`base URL <domain-name/web-base-url>` according to
   the company in use.
