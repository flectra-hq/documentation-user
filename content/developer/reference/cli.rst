...
The CLI :dfn:`command-line interface` offers several functionalities related to Flectra. You can use it
to :ref:`run the server <reference/cmdline/server>`, :ref:`launch Flectra as a Python console
environment <reference/cmdline/shell>`, :ref:`scaffold an Flectra module <reference/cmdline/scaffold>`,
:ref:`populate a database <reference/cmdline/populate>`, or :ref:`count the number of lines of code
<reference/cmdline/cloc>`.

.. important::
   The command to use to call the CLI depends on how you installed Flectra. In the examples below, we
   assume that you are :ref:`running Flectra from source <setup/install/source>` with the
   :file:`flectra-bin` file. If you installed Flectra :ref:`from a distribution package
   <setup/install/packaged>` or :ref:`with Docker <setup/install/docker>`, you must adapt the
   command.

   .. tabs::

      .. tab:: Run Flectra from source

         #. Navigate to the root of the directory where you downloaded the source files of Flectra
            Community.
         #. Run all CLI commands with :command:`./flectra-bin`

      .. tab:: Flectra was installed from a distribution package

         When Flectra was installed, an executable named `flectra` was added to your user's PATH. Replace
         all occurrences of :command:`flectra-bin` with :command:`flectra` in the examples below.

      .. tab:: Flectra was installed with Docker

         Please refer to the `documentation of the official Docker image of Flectra
         <https://hub.docker.com/_/flectra/>`_.
...
.. option:: --version
   ...
   shows Flectra version e.g. "Flectra Server {BRANCH}"

.. tip:: You can enable auto-completion in your shell by running

  .. code-block:: bash

    echo "complete -W '`./flectra-bin --help | \
      sed -e 's/[^a-z_-]\(-\+[a-z0-9_-]\+\)/\n\1\n/' | \
      grep -- '^-' | sort | uniq | tr '\n' ' '`' flectra-bin" >> ~/.bash_completion
...
.. option:: --load <modules>
   ...
   installed (i.e. the majority of Flectra addons). Default is ``base,web``.
...
.. option:: -D <data-dir-path>, --data-dir <data-dir-path>
   ...
   directory path where to store Flectra data (eg. filestore, sessions).
   If not specified, Flectra will fallback
   to a predefined path. On Unix systems its
   one defined in ``$XDG_DATA_HOME`` environmental variable
   or :file:`~/.local/share/Flectra` or :file:`/var/lib/Flectra`.
...
.. option:: --test-tags [-][tag][/module][:class][.method]
    ...
    * The tag will match tags added on a class with a :func:`~flectra.tests.common.tagged` decorator
...
.. code-block:: console
    ...
    $ flectra-bin scaffold my_module /addons/
