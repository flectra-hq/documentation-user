...
For most Flectra code, we want to use a module system.  Because of the way assets
work in Flectra (and in particular, the fact that each installed flectra addon can
modify the list of files contained in a bundle), Flectra has to resolve modules
browser side.  To do that, Flectra provides a small module system described just
below (see :ref:`frontend/modules/odoo_module`).

However, Flectra also provides support for native javascript modules (see
:ref:`frontend/modules/native_js`). These modules
will simply be translated by the server into flectra modules. It is encouraged to
write all javascript code as a native module, for a better IDE integration. In
the future, the Flectra module system should be considered an implementation detail,
