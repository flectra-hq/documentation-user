...
The Flectra Javascript framework uses a custom component framework called Owl. It
...
`Owl documentation <https://github.com/flectra/owl/blob/master/doc/readme.md>`_
...
.. important::
   ...
   through a pull request on https://github.com/flectra/owl.

.. note::
   Currently, all Flectra versions (starting in version 14) share the same Owl version.

Using Owl components
====================

The `Owl documentation`_ already documents in detail the Owl framework, so this
page will only provide Flectra specific information. But first, let us see how we
can make a simple component in Flectra.
...
`owl`: it can simply be used like most libraries in Flectra. Note that we
defined here the template as a static property, but without using the `static`
keyword, which is not available in some browsers (Flectra javascript code should
...
Flectra should be defined in an xml file, so they can be translated. In that case,
...
Flectra code is not yet completely made in Owl, so it needs a way to tell the
...
is an occasionally useful pattern in Flectra, we need to make sure that no component
in Flectra directly uses the constructor method. Instead, components should use the
`setup` method:
...
`addon_name.ComponentName`. This prevents name collision between flectra addons.

Reference List
==============

The Flectra web client is built with `Owl <https://github.com/flectra/owl>`_ components.
To make it easier, the Flectra javascript framework provides a suite of generic
...
To solve these issues once and for all, the Flectra framework provides a set of two
...
`OWL slots <https://github.com/flectra/owl/blob/master/doc/reference/slots.md>`_ :
