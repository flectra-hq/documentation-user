...
way to perform a request to the Flectra server.
...
.. js:data:: async
    ...
    functions to call the Flectra server.
...
While most interactions between the client and the server in flectra are `RPCs` (`XMLHTTPRequest`), lower level
...
.. note::
    ...
    only be used to interact with Flectra controllers. To work with models (which
...
* either the flectra server returns an error (so, we call this a ``server`` error).
...
if the selector contained in its href attribute is valid to distinguish anchors and Flectra
...
title. For example, if the current document title is "Flectra", we can change it
to "Flectra 15 - Apple" by using the following command:
