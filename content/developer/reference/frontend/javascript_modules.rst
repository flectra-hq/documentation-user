...
Flectra supports three different kinds of javascript files:
...
- :ref:`Flectra modules <frontend/modules/odoo_module>` (using a custom module system),
...
Note that native javascript files are processed by the Flectra server and transformed into Flectra custom modules.
...
.. note::
  In Flectra, all external libraries are loaded as plain javascript files.
...
Most new Flectra javascript code should use the native javascript module system. This
is simpler, and brings the benefits of a better developer experience with a better
integration with the IDE.

There is a very important point to know: Flectra needs to know which files
should be translated into :ref:`Flectra modules <frontend/modules/odoo_module>` and which
files should not be translated. This is an opt-in system: Flectra will look at the
first line of a JS file and check if it contains the string *@flectra-module*. If so, it will
automatically be converted to an Flectra module.
...
error). This file will then be translated into an Flectra module that look like this:

.. code-block:: javascript

   flectra.define('@web/file_a', function (require) {
   'use strict';
   let __exports = {};
...
So, as you can see, the transformation is basically adding `flectra.define` on top,
...
will be converted as well. Every file located in an Flectra addon
...
Relative imports work, but only if the modules are in the same Flectra addon. So, imagine that we have
...
Because :ref:`Flectra modules <frontend/modules/odoo_module>` follow a different module naming pattern, a system exists to allow a smooth
...
.. code-block:: javascript
  ...
  /** @flectra-module alias=web.someName**/
  import { someFunction } from './file_b';
...
.. code-block:: javascript
  ...
  flectra.define(`web.someName`, function(require) {
...
.. code-block:: javascript
  ...
  /** @flectra-module alias=web.someName default=0**/
  import { someFunction } from './file_b';
...
For performance reasons, Flectra does not use a full javascript
...
- Flectra needs a way to determine if a module is described by a path (like :file:`./views/form_view`)
  or a name (like `web.FormView`). It has to use a heuristic to do just that: if there is a `/` in
  the name, it is considered a path.  This means that Flectra does not really support module names with
  a `/` anymore.
...
within the same Flectra addon.


.. _frontend/modules/odoo_module:

Flectra Module System
=====================

Flectra has defined a small module system (located in the file
:file:`addons/web/static/src/js/boot.js`, which needs to be loaded first). The Flectra
