...
Flectra provides support for testing modules using `Python's unittest library
...
to run on the command-line. It implies :option:`--test-enable <flectra-bin --test-enable>`,
so it's not necessary to specify :option:`--test-enable <flectra-bin --test-enable>`
when using :option:`--test-tags <flectra-bin --test-tags>`.
...
.. code-block:: console
    ...
    $ flectra-bin --test-tags nice,standard
...
.. code-block:: console
    ...
    $ flectra-bin --test-tags 'standard,-slow'
...
test. The full syntax of the format accepted by :option:`--test-tags <flectra-bin --test-tags>`
is:
...
So if you want to test the `stock_account` module, you can use:
    ...
    .. code-block:: console
        ...
        $ flectra-bin --test-tags /stock_account
...
directly:
    ...
    .. code-block:: console
        ...
        $ flectra-bin --test-tags .test_supplier_invoice_forwarded_by_internal_user_without_supplier

This is equivalent to

    .. code-block:: console

        $ flectra-bin --test-tags /account:TestAccountIncomingSupplierInvoice.test_supplier_invoice_forwarded_by_internal_user_without_supplier
...
- ``standard``: All Flectra tests that inherit from
  :class:`~flectra.tests.common.BaseCase` are implicitly tagged standard.
...
.. important::
    ...
    :option:`-i <flectra-bin -i>` switch at least once. After that it's no longer
    needed, unless you need to upgrade the module, in which case
    :option:`-u <flectra-bin -u>` can be used. For simplicity, those switches are
...
:class:`~flectra.tests.common.HTTPCase`, and call `start_tour`:
