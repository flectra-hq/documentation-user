...
Depending on the intended use case, there are multiple ways to install Flectra. This tutorial will
stick to the :ref:`source install <setup/install/source>` (:dfn:`running Flectra from the source
code`), which is best suited for Flectra developers.

Throughout this document, we assume that you are installing your development environment on a
laptop provided by Flectra with Linux Mint installed and up-to-date. If that is not the case, switch to
...
<https://git-scm.com/>`_ because the Flectra source code is managed on `GitHub <https://github.com/>`_.
...
You need a GitHub account to fetch the sources and contribute to Flectra's development. If you don't
have one yet, create it. For the username, we recommend using your trigram "xyz" (or quadrigam)
followed by '-flectra': 'xyz-flectra'.
...
It is time to fetch the source code of Flectra. First, let's create a home for the Git repositories in
:file:`$HOME/src/`.
...
Then, clone the two repositories with SSH as explained in the :ref:`Installing Flectra guide
...
To contribute to an Flectra repository, you first need to `fork it
...
.. tip::
   If you are lucky enough to work at Flectra, the forks already exist. They are hosted on
   https://github.com/flectra-dev/flectra and https://github.com/flectra-dev/enterprise.

After your two forks are created, their remote address can be added in your local repositories. In
the commands below, replace `flectra-dev/flectra` and `flectra-dev/enterprise` with the name of your forks if
needed.
...
As seen in :ref:`tutorials/getting_started/01_architecture`, Flectra's server runs on Python and uses
PostgreSQL as an RDBMS. In the context of a development machine, the easiest approach is to install
everything locally. To do so, follow once again the :ref:`Installing Flectra guide
...
Once all dependencies are set up, Flectra can be launched by running `flectra-bin`, the command-line
...
.. tip::
   ...
   - Other commonly used arguments are:
     ...
     - :option:`-i <flectra-bin --init>`: Install some modules before running the server
       (comma-separated list).
     - :option:`-u <flectra-bin --update>`: Update some modules before running the server
       (comma-separated list).
...
- | Switch branches:
  | When you switch branches, both repositories (flectra and enterprise) must be synchronized, i.e.
...
- Fetch and rebase:
  ...
  .. code-block:: console
     ...
     $ cd $HOME/src/flectra
     $ git fetch --all --prune
     $ git rebase --autostash flectra/{BRANCH}
...
If you are working at Flectra, many of your colleagues are using `VSCode
...
semantic warnings or errors. Flectra source code tries to respect Python's and JavaScript's standards,
...
<https://github.com/flectra/flectra/wiki/Javascript-coding-guidelines#use-a-linter>`_.
