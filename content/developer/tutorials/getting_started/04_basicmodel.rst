...
create an Flectra module. However, at this point it is still an empty shell which doesn't allow us to
store any data. In our real estate module, we want to store the information related to the
properties (name, description, price, living area...) in a database. The Flectra framework provides
...
A key component of Flectra is the `ORM`_ layer.
...
the model in the Flectra system. Here is a minimum definition of a
model::
...
Any modification of the Python files requires a restart of the Flectra server. When we restart
the server, we will add the parameters ``-d`` and ``-u``:

.. code-block:: console

    $ ./flectra-bin --addons-path=../custom,../professional/,addons -d rd-demo -u estate
...
.. code-block:: text
    ...
    ...
    WARNING rd-demo flectra.models: The model estate.property has no _description
    ...
    WARNING rd-demo flectra.modules.loading: The model estate.property has no access rules, consider adding one...
    ...
...
The ``name`` field is a :class:`~flectra.fields.Char` which will be represented as a Python
...
Simple field examples are :class:`~flectra.fields.Boolean`, :class:`~flectra.fields.Float`,
:class:`~flectra.fields.Char`, :class:`~flectra.fields.Text`, :class:`~flectra.fields.Date`
and :class:`~flectra.fields.Selection`.
...
:attr:`~flectra.fields.Field.string` (``str``, default: field's name)
    The label of the field in UI (visible by users).
:attr:`~flectra.fields.Field.required` (``bool``, default: ``False``)
    If ``True``, the field can not be empty. It must either have a default
    value or always be given a value when creating a record.
:attr:`~flectra.fields.Field.help` (``str``, default: ``''``)
