...
Twig (PHP). Flectra comes with its own built-in engine: :ref:`reference/qweb`.
QWeb is the primary templating engine used by Flectra. It is an XML templating engine and used
mostly to generate HTML fragments and pages.

You probably already have come across the `kanban board`_ in Flectra where the records are
...
views are a standard Flectra view (like the form and list views), but their structure is much more
...
- ``record``: an object with all the requested fields as its attributes. Each field has
  ...
  user parameters and the latter is the direct value from a :meth:`~flectra.models.Model.read`.
