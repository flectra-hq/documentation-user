...
intended to store business data. In a business application such as Flectra, one of the first questions
to consider is who\ [#who]_ can access the data. Flectra provides a security mechanism to allow access
...
<{GITHUB_PATH}/flectra/addons/base/data/res.country.state.csv>`_ which is loaded at installation of the
`base` module.
...
.. note::
    ...
    **Goal**: at the end of this section, the following warning should not appear anymore:
    ...
    .. code-block:: text
        ...
        WARNING rd-demo flectra.modules.loading: The models ['estate.property'] have no access rules...

When no access rights are defined on a model, Flectra determines that no users can access the data.
It is even notified in the log:

.. code-block:: text

    WARNING rd-demo flectra.modules.loading: The models ['estate.property'] have no access rules in module estate, consider adding some, like:
...
.. [#who] meaning which Flectra user (or group of users)
