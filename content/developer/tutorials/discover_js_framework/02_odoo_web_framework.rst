...
learn how to use the Flectra JavaScript framework which is is built on top of Owl.
...
opportunity to discover many useful features in the Flectra JavaScript framework.
...
Most screens in the Flectra web client uses a common layout: a control panel on top, with some buttons,
...
Bafien Carpink want buttons for easy access to common views in Flectra. In order to do that, you will
need to use the action service.

:ref:`Services <frontend/services>` is a notion defined by the Flectra JavaScript framework, it is a
...
.. exercise::
   ...
   #. A button `Customers`, which opens a kanban view with all customers (this action already
      ...
      <{GITHUB_PATH}/flectra/addons/base/views/res_partner_views.xml#L513>`_).
