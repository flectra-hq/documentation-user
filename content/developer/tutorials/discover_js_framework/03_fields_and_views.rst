...
Fields and views are among the most important concepts in the Flectra user interface. They are key to
...
Flectra form views support a `widget` API, which is like a field, but more generic. It is useful to
...
Views are among the most important components in Flectra: they allow users to interact with their
data. Let us discuss how Flectra views are designed.

The power of Flectra views is that they declare how a particular screen should work with an XML
...
Most (or all?) Flectra views share a common architecture:
...
.. exercise::
   ...
   #. Add a `js_class` attribute to the arch of the form view so Flectra will load it.
