...
.. warning::
    ...
    2.0-core from the repository XXX, it contains a version of the module
    created during the core training we can use as a starting point.
...
For most Flectra applications [#app]_ a good baseline is to have *user* and
...
In keeping with Flectra's data-driven nature, a group is no more than a record of
...
.. exercise::
    ...
    .. tip::
        ...
        Since we modified data files, remember to restart Flectra and update the
...
access, which means we need to *bypass* the normal security checks of Flectra in
order to create an invoice *despite* the current user not having the right to
do so.

There are two main ways to bypass existing security checks in Flectra, either
...
In Flectra, access rights and access rules are only checked *when performing data
...
You should see ``reached`` in your Flectra log, followed by an access error.
...
Flectra can be used to manage multiple companies inside the same system, however
the actual handling is up to individual modules: Flectra itself provides the tools
...
Specific Flectra models can be associated directly with groups (or companies, or
...
* *Visibility* features mean a user can still access the model or record
  ...
  operations remotely using RPC <../../webservices/flectra>`, things might just not be
...
.. [#app] An Flectra Application is a group of related modules covering a business
