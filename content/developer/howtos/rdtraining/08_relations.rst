...
salesperson must be an employee of the real estate agency (i.e. an Flectra user).

In Flectra, there are two models which we commonly refer to:
...
- ``res.users``: the users of the system. Users can be 'internal', i.e. they have
  access to the Flectra backend. Or they can be 'portal', i.e. they cannot access the backend, only the
...
**Reference**: the documentation related to this topic can be found in
:class:`~flectra.fields.Many2many`.
...
.. danger::
    ...
    there *must* be a :class:`~flectra.fields.Many2one` field defined in the comodel.
...
views. How does Flectra know which property our offer is linked to? Well that's part of the
magic of using the Flectra framework: sometimes things are defined implicitly. When we create
