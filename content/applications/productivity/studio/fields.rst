...
From a technical point of view, there are 15 field types in Flectra. However, you can choose from 20
...
- :guilabel:`Image`: displays an image using a URL. The value cannot be edited manually, but a
  ...
  .. note::
     ...
     <studio/fields/simple-fields/image>` directly, as the image is not stored in Flectra when using a
...
- :guilabel:`Phone`: the value becomes a clickable *tel* link.
  ...
  .. tip::
     Tick :guilabel:`Enable SMS` to add an option to send an SMS directly from Flectra next to the
     field.
...
.. note::
   ...
   field if none exists already on the model. Flectra offers to add the :guilabel:`Currency` field for
...
The :guilabel:`Html` field is used to add text that can be edited using the Flectra HTML editor.

- :guilabel:`Multiline Text`: disables the Flectra HTML editor to allow editing raw HTML.
