=============================================
Set up Sendcloud shipping services in Flectra
=============================================

Sendcloud is a shipping service aggregator that facilitates the integration of European
shipping carriers with Flectra. Once integrated, users can select shipping carriers on inventory
operations in their Flectra database.

.. seealso::
   `Sendcloud integration documentation <https://support.sendcloud.com/hc/en-us/articles
   /360059470491-Flectra-integration>`_
...
.. note::
   ...
   in the Flectra database.

.. important::
   Flectra integration of Sendcloud does **not** work on free plans of Sendcloud.
...
Flectra warehouse name should be entered here, and the characters should be exactly the same.
...
.. example::
   ...
   | **Flectra warehouse configuration**
   ...
   Notice how the inputs for the :guilabel:`Warehouse` field, for both the Flectra configuration and
...
right. Next, search for :guilabel:`Flectra Native`. Then, click on :guilabel:`Connect`.
...
:guilabel:`Integration`. The naming convention is as follows: `Flectra CompanyName`, with the user's
company name replacing `CompanyName` (e.g. `Flectra StealthyWood`).
...
After the Sendcloud account is set up and configured, it's time to configure the Flectra database.
To get started, go to Flectra's :guilabel:`Apps` module,  search for the :guilabel:`Sendcloud
Shipping` integration, and install it.

.. image:: sendcloud_shipping/sendcloud-mod.png
   :align: center
   :alt: Sendcloud Shipping module in the Flectra Apps module.
...
.. example::
   Sample Sendcloud shipping products configured in Flectra:
...
.. image:: sendcloud_shipping/sendcloud-example.png
   :align: center
   :alt: Example of shipping products configured in Flectra.

.. tip::
   Sendcloud does not provide test keys when a company tests the sending of a package in Flectra. This
   ...
   Flectra has a built-in layer of protection against unwanted charges when using test environments.
...
When creating a quotation in Flectra, add shipping and a :guilabel:`Sendcloud shipping product`. Then,
...
when creating a quotation in Flectra, then the pricing information needs to be updated in Sendcloud.
