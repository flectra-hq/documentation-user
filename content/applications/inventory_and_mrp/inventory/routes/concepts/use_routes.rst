...
In Flectra, a product's supply chain strategy can be configured using *Routes*, which feature *Pull
...
Pull Rules are used to fulfill a sales order. Flectra generates a need at the *Customer Location* for
each product in the order. Because pull rules are triggered by a need, Flectra looks for a pull
...
Then, Flectra finds another pull rule that tries to fulfill the need for the *Shipping Area*: the
...
.. note::
   All these product transfers are automatically generated by Flectra based on the pull rules, starting
...
Since *Routes* are a collection of *Push and Pull Rules*, Flectra helps you manage advanced route
...
.. image:: use_routes/multi-steps-routes-feature.png
   :align: center
   :alt: Activate the Multi-Step Routes feature in Flectra Inventory.

.. note::
   The :guilabel:`Storage Locations` feature is automatically activated with the
   :guilabel:`Multi-Step Routes` feature.

Once this first step is completed, the user can use pre-configured routes that come with Flectra, or
they can create custom routes.

Pre-configured routes
---------------------

To access Flectra's pre-configured routes, go to :menuselection:`Inventory --> Configuration -->
...
.. image:: use_routes/example-preconfigured-warehouse.png
   :align: center
   :alt: A preconfigured warehouse in Flectra Inventory.
...
--> Routes` to see the specific routes that Flectra generated.

.. image:: use_routes/preconfigured-routes.png
   :align: center
   :alt: View of all the preconfigured routes Flectra offers.
...
.. image:: use_routes/rules-example.png
   :align: center
   :alt: An example of rules with push & pull actions in Flectra Inventory.
...
The available rules trigger various actions. If Flectra offers *Push* and *Pull* rules, others are
...
- :guilabel:`Pull From`: this rule is triggered by a need for the product in a specific location.
  ...
  specific component. When the need appears in the destination location, Flectra generates a picking to
  fulfill this need.
- :guilabel:`Push To`: this rule is triggered by the arrival of some products in the defined source
  location. In the case of the user moving products to the source location, Flectra generates a picking
...
In the *Scheduling* section, you can determine how Flectra behaves when one of the chain's pickings is
