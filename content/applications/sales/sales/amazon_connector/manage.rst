===============================
Manage Amazon orders in Flectra
===============================

Synchronization of orders
=========================

Orders are automatically fetched from Amazon and synchronized in Flectra at regular intervals.
...
created in Flectra if they are not yet registered.

When an order is canceled in Amazon and was already synchronized in Flectra, the corresponding sales
order is automatically canceled in Flectra.
...
When a **FBM** (Fulfilled by Merchant) order is synchronized in Flectra, a picking is created along
...
.. tip::
   - If your chosen carrier isn't one supported by Flectra, you can still create a carrier bearing its
...
When a **FBA** (Fulfilled by Amazon) order is synchronized in Flectra, a stock move is recorded for
...
.. tip::
   To follow your Amazon (FBA) stock in Flectra, you can make an inventory adjustment after
   replenishing it. You can also trigger an automated replenishment from reordering rules on the
   Amazon location.

.. tip::
   The Amazon location is configurable by Amazon account managed in Flectra. All accounts of the same
...
You can issue invoices for Amazon orders in Flectra. Click **Create Invoice** in the sales order to do
...
Flectra, you simply credit the Amazon intermediary account by the amount received.
