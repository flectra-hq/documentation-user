...
#. :guilabel:`Generate Certificate Sign Request (Flectra)`. When this option is selected, a file with
...
#. :guilabel:`Upload Certificate and Private Key (Flectra)`. Once the certificate is generated, upload
   it to Flectra using the :guilabel:`Pencil` icon next to the field :guilabel:`Certificado` and select
...
.. tip::
   ...
   <http://www.afip.gob.ar/ws/documentacion/certificados.asp>`_. Furthermore, Flectra allows the user
...
.. image:: argentina/identification-types.png
   :align: center
   :alt: A list of AR Localization document types in Flectra, as defined by AFIP.

.. note::
   The complete list of :guilabel:`Identification Types` defined by the AFIP is included in Flectra,
...
.. image:: argentina/automatic-tax-configuration.png
   :align: center
   :alt: A list of AR Localization taxes with financial amount and configuration in Flectra.
...
labeled as inactive in Flectra by default. Before creating a new tax, be sure to check if that tax is
not already included as inactive.

.. image:: argentina/special-inactive-taxes.png
   :align: center
   :alt: A list showing less common Argentinean tax options, which are labeled as inactive in Flectra
         by default.
...
.. image:: argentina/default-document-type-info.png
   :align: center
   :alt: A list of document types in Flectra.
...
.. image:: argentina/sales-journal.png
   :align: center
   :alt: A AFIP POS System field which is available on Sales journals in Flectra.
...
For the first invoice, Flectra synchronizes with the AFIP automatically and displays the last sequence
used.
...
:guilabel:`Cliente / Proveedor del Exterior` - :guilabel:`Ley N° 19.640`, Flectra automatically
assigns the:
...
.. image:: argentina/export-invoice.png
   :align: center
   :alt: Export invoice fields autofilled in Flectra.
...
Number as a reference for any possible issues on the sequence synchronization between Flectra and
AFIP.
...
Validate vendor bills in Flectra
********************************

With the vendor validation settings enabled, a new button shows up on the vendor bills inside of
Flectra, labeled :guilabel:`Verify on AFIP`, which is located next to the :guilabel:`AFIP
