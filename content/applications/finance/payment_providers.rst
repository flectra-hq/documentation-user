...
Flectra embeds several **payment providers** that allow your customers to pay on their *Customer
...
.. note::
   Flectra apps delegate the handling of sensitive information to the certified payment provider so
   that you don't ever have to worry about PCI compliance.

   This means that no sensitive information (such as credit card numbers) is stored on Flectra servers
   or Flectra databases hosted elsewhere. Instead, Flectra apps use a unique reference number to the data
...
.. tip::
   ...
   or Adyen, Flectra bundles the :doc:`Demo payment provider <payment_providers/demo>`. This payment
...
**payment token** is created in Flectra and can be used as a payment method for subsequent payments
...
.. note::
   You remain fully PCI-compliant when you enable this feature because Flectra does not store the card
...
.. note::
   ...
   - Flectra does not support this feature for all payment providers but some allow the manual capture
...
If your payment provider supports this feature, you can refund payments directly from Flectra. It does
...
.. note::
   ...
   - Flectra does not support this feature for all payment providers but some allow to refund payments
     from their website interface.
...
.. image:: payment_providers/activation.png
   :align: center
   :alt: Click on install, then on activate to make the payment provider available on Flectra.
...
the credentials required for third-party use. Flectra requires these credentials to communicate with
