...
With the Mexican location in Flectra you will not only be able to comply with the legal requirements to
invoice in Mexico, but also use it as your accounting system, satisfying the normal needs of the
market. This makes Flectra the perfect solution to manage your business in Mexico.

Pre requirements
================

Before installing the modules and making the necessary configurations to have the Mexican
localization in Flectra, it is necessary to meet the following requirements:
...
#. Choose a PAC and purchase stamps. Currently the Mexican location in Flectra works with two PACs:
...
#. Have knowledge and experience with billing, sales and accounting in Flectra. This documentation
   contains only the information necessary to enable the use of Flectra in a company based in Mexico.
...
#. | **Flectra Mexican localization reports (l10n_mx_reports)**
...
#. | **EDI External Trade Complement for Mexico (l10n_mx_edi_external_trade)**
   | For clients that export, add the foreign trade complement to the CFDI, and the logic for
     filling it.
#. | **Flectra Mexico Localization for Invoice with customs Number (l10n_mx_edi_customs)**
...
#. | **Flectra Mexico Localization for Stock / Landing (l10n_mx_edi_landing)**
...
#. | **Flectra Mexico Localization for Sale Coupon (l10n_mx_edi_sale_coupon)**
   | Complements the Flectra coupon module (*sale_coupon*) to avoid errors in the generation of CFDIs.
...
normal billing flow in Flectra.
...
.. image:: media/mx_co_02.png
   :align: center
   :alt: Update the company's details in the Settings of Flectra
...
.. warning::
   ...
   countries that Flectra shows, because if it is entered manually there is a risk of creating a "new
...
done through a pre-existing field in Flectra called "Fiscal Regime".
...
.. image:: media/mx_co_04.png
   :align: center
   :alt: Set the Fiscal Regime in Flectra Accounting
...
When creating a contact to be invoiced in Flectra, the following information must be configured for
...
A necessary configuration for electronic invoicing to work correctly in Flectra is to add the factor
...
Flectra, or those that you add that are necessary for your company bill.

.. tip::
   For the 0% VAT tax, select the option *Exento* instead of *Tasa* within the **Factor Type**
   field.

When registering a payment, Flectra will carry out the movement of taxes from the **Cash Basis
...
Another important step to configure electronic invoicing in Flectra is to enter the PAC which you are
...
.. warning::
   ...
   invoices from Flectra. We have the following PACs available: `Quadrum
...
The invoicing process in Flectra is based on `Annex 20
...
from Flectra, by clicking the *Send and Print* button. You can also download the PDF file directly to
...
The foreign trade invoicing process in Flectra is based on the corresponding `SAT regulation
...
can also configure Flectra to record the process.

First, go to :menuselection:`Apps`, remove the "Apps" filter and search for ``Mexico``, ``mx`` or
``l10n_mx``. Then, install **Flectra Mexico Localization for Stock / Landing module
(l10n_mx_edi_landing)**.
...
The **Payment Terms** are already configured in Flectra when installing the Mexican localization, this
means that if you go to :menuselection:`Accounting --> Configuration --> Payment Terms`, you will
find the default list in Flectra.
...
date expiration date for your invoice and Flectra will detect if it is after the first day of the
...
of payments: **PUE** or **PPD**. In both cases the payment process in Flectra is the same, the
...
If the payment is a PPD type, Flectra will generate the corresponding payment complement automatically
...
Add the down payment product as default to be used from the Flectra configurations.
...
#. Click on *Request EDI Cancellation* to inform the SAT that you want to cancel the invoice, in
   ...
   invoicing** field in Flectra changes to *To Cancel*)
...
#. Flectra synchronizes with the SAT to update the status of the **Electronic invoicing** with a
   scheduled action, Invoices canceled in the SAT will be canceled in Flectra.
...
Once canceled in the SAT, Flectra will synchronize the status of the SAT through scheduled actions that
are executed every day to synchronize the statuses of the SAT, Electronic invoicing and Flectra (this
scheduled action can be executed manually by entering with developer mode).

If the invoice is canceled in the SAT, in Flectra it is also canceled, which allows you to switch the
...
declared in Financial Reports and to the government. In Flectra, when canceling an invoice, the journal
...
If the accounting period is closed due to the blocking dates, Flectra will not allow to modify or add
...
If the accounting period is closed, and the invoice was canceled in the SAT, the status in Flectra will
...
Accounting for Mexico in Flectra is composed of 3 reports:
...
All normal analysis and listed functions are available here as well as any normal Flectra Report.
...
How to generate this report in Flectra?
***************************************
...
- There are 3 VAT options for this report, 16%, 0% and exempt, one invoice line in Flectra is
...
- Remember that to pay an invoice that represents a prepayment, you must first request the invoice
  and then pay it and properly reconcile the payment following the standard Flectra procedure.
...
Closing Fiscal Period in Flectra
--------------------------------
...
- Reconcile all accounts on your **Balance Sheet**
  ...
  - Compare your bank balances in Flectra against the current bank balances on your statements. Use
...
In Flectra there is no need to make a specific year-end entry to close the reporting income accounts
...
to the closing date of the year that you specify in Flectra. In addition, at any time that you
...
SAT without having to manually create this information every day in Flectra.
...
.. image:: media/mx-xsd-cfdi.png
   :align: center
   :alt: Download XSD files to CFDI from the Companies list view on Flectra
