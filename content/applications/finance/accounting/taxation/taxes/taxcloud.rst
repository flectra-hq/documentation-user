...
#. Select a :guilabel:`Default Category` and :guilabel:`Save`. The :guilabel:`Default Category` is
   ...
   .. image:: taxcloud/taxcloud-settings.png
      :align: center
      :alt: Filling in TaxCloud API Keys in Flectra
...
.. note::
   ...
   Category`, Flectra only considers the :guilabel:`TaxCloud Category` found on the product itself.
...
Sales taxes are calculated in Flectra based on :doc:`fiscal positions <fiscal_positions>`. A fiscal
position for the United States is created when enabling TaxCloud.

You can configure Flectra to automatically detect to which customers the fiscal position should be
...
in the price sent to TaxCloud. However, in Flectra, the discount is on another line entirely. Still,
