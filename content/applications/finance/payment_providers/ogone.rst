...
You need to create an **API user** to be used in the creation of transactions from Flectra. While you
can use your main account to do so, using an **API user** ensures that if the credentials used in
Flectra are leaked, no access to your Ogone configuration is possible. Additionally, passwords for
...
Set up Ogone for Flectra
------------------------

Ogone must now be configured to accept payments from Flectra. Head to :menuselection:`Configuration -->
...
.. tip::
   If you need to use another algorithm, such as `sha-1` or `sha-256`, within Flectra, activate the
...
used to digitally sign the transaction requests and responses between Flectra and Ogone. By using these
...
configuration, as Flectra only allows a single **SHA-IN** and single **SHA-OUT** passphrase.
...
- The :guilabel:`URL` fields for :guilabel:`HTTP redirection in the browser` can be left empty, as
  Flectra will specify these URLs for every transaction request.
...
To set up Ogone in Flectra, head to :menuselection:`Accounting --> Configuration --> Payment Providers`
