...
Flectra. It works as a seamless flow where the customer is routed to the Paypal website to register the
payment.
...
Flectra needs your **API Credentials** to connect with your PayPal account, which comprise:
...
.. important::
   ...
   change the **State** to *Test Mode*. We recommend doing this on a test Flectra database rather than
...
To activate this, go to Paypal configuration's :guilabel:`Fees` tab in Flectra and activate
...
First, set up your Paypal account to build a seamless customer experience with Flectra.
...
The *Auto Return* feature automatically redirects your customers to Flectra once the payment is
processed.

From the *Website payments* settings page, go to :menuselection:`Website preferences --> Update -->
Auto return for website payments` and select **On**. Enter the address of your Flectra database (e.g.,
`https://yourcompany.flectra.com`) in the **Return URL** field.

.. note::
   Any URL will do the job. Flectra only needs the setting to be enabled since it uses another URL.
...
request sent by Flectra to Paypal. Otherwise, some transactions fail without notice.
...
Thanks to Paypal Sandbox accounts, you can test the entire payment flow in Flectra.
...
Enter your sandbox credentials in Flectra and ensure Paypal is set on *Test Mode*. We recommend doing
this on a test Flectra database rather than your main database.
